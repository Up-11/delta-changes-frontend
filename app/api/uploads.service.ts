...
export const uploadsService = {
  ...
  async uploadImage(file: File): Promise<UploadResponse> {
    ...
    return api.upload<UploadResponse>(API_ENDPOINTS.uploads.uploadImage.path, formData)
  },
  ...
  async uploadVideo(file: File): Promise<UploadResponse> {
    const formData = new FormData()
    formData.append('file', file)
    return api.upload<UploadResponse>(API_ENDPOINTS.uploads.uploadVideo.path, formData)
  },
  ...
  async uploadMultipleImages(files: File[]): Promise<UploadResponse[]> {
    ...
    return api.upload<UploadResponse[]>(API_ENDPOINTS.uploads.uploadMultipleImages.path, formData)
  },
  ...
  async uploadMultipleVideos(files: File[]): Promise<UploadResponse[]> {
    const formData = new FormData()
    files.forEach(file => formData.append('files', file))
    return api.upload<UploadResponse[]>(API_ENDPOINTS.uploads.uploadMultipleVideos.path, formData)
  },
  ...
  getFileUrl(filename: string): string {
    return `${API_ENDPOINTS.static.uploads.base}/${filename}`
  }
}
